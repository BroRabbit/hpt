...
is not defined in the HPT configuration file, then this leads to the
crush of the whole Perl hook.
...
sub route_to()
{
	...
	if ( defined( $links{$1}{password} ) ) {
	    ...
	    $route =~ /\d+\:\d+\/\d+(\.?\d*)/;
	    $route .= '.0' unless defined( $1 );
	    $text =~ s/\r\%RouteTo\:\s+(\d+\:\d+\/\d+\.?\d*)\s*(\d+\:\d+\/\d+){0,1}/\rRouted To\: $1 at @{$config{addr}}[0]/i;
	    $change=1;
	} else {
